...
function obtenerLibrerias(carpeta, nombreArchivo) {
  ...
  //   console.log(renglones.length);
  //   return;
  const Nrenglones = renglones.length;
  let aux = [];
  imports.map((a) => {
  ...
  carpetas.forEach((nombreCarpeta) => {
    if (nombreCarpeta != "node_modules") {
      ...
      archivosRaiz.forEach((nombreArchivo) => {
        let { librerias, Nrenglones } = obtenerLibrerias(
          nombreCarpeta,
          nombreArchivo
        );
        if (Nrenglones)
          console.log("   - " + nombreArchivo + " + " + Nrenglones);
